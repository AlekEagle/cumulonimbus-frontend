<template>
  ...
  <div class="quick-action-buttons-container">
    <BackButton fallback="/dashboard/set-up" />
  </div>
  <Modal ref="identityModal" title="Verify your identity">
    <p>
      ...
      identifiable in case the session needs to be invalidated.
    </p>
...
@Options({
  components: { ContentBox, Modal, Loading, BackButton },
  data() {
    return {
...
export default class SetUpService extends Vue {
  ...
  copyToClipboard() {
    navigator.clipboard.writeText(this.$data.newIdentity?.token as string).then(
      () => {
        (this.$parent?.$parent as App).temporaryToast(
